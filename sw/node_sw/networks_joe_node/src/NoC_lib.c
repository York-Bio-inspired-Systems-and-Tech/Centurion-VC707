...
void NoC_Write_Node_Packet(int node, Xuint8* data, int length, Xuint32 header)
{
	...
	else
	{
		...
		for(i=0; i<delta_y; i++)
		{
			NOC_TX_BUFF[buff_index] = 0x1C2;
			buff_index++;
		}
	}

	//add an internal to reach the node
	NOC_TX_BUFF[buff_index] = 0x1C4;
	buff_index++;
...
void Debug_Write_Safe(Xuint8 data)
{
        ...
        while((DEBUG_IN & 0x1FF) != (0x100 | data));
...
volatile Xuint16 experiment_log_index __attribute__ ((section (".noinit")));
...
void Log_Action(Node_Action action, Xuint8 p0, Xuint8 p1, Xuint8 p2)
{
        ...
        {
        	 Xuint32* data_addr =  log_entry;
			Xuint32 time = Read_RTC();
			data_addr[0] = (p2 << 24) | (p1 << 16) | (p0 << 8) | action;
			data_addr[1] = time;
			xil_printf("logged to %x\n", data_addr);
