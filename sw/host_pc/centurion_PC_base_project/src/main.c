...
int main()
{
	...
	Centurion_Reset_NoC();
	Centurion_Write_Reg(CENT_REG_NODE_DEBUG_SEL, 2);
	printf("node 0 status %x\n", Centurion_Read_Reg(CENT_REG_NOC_DEBUG_DATA));
		printf("node 0 status %x\n", Centurion_Read_Reg(CENT_REG_NOC_DEBUG_DATA));
		while(1);
...
*/
	...
	printf("node 0 status %x\n", Centurion_Read_Reg(CENT_REG_NOC_DEBUG_DATA));
	Centurion_Write_Reg(CENT_REG_NODE_UART_SEL, 0);
	Centurion_Write_Reg(CENT_REG_NODE_DEBUG_SEL, 1);
	Centurion_Write_Reg(CENT_REG_NODE_DEBUG_CMD, 0xED);

	Centurion_Reset_NoC();
	while(1);
	printf("node 0 status %x\n", Centurion_Read_Reg(CENT_REG_NOC_DEBUG_DATA));
	printf("node 0 status %x\n", Centurion_Read_Reg(CENT_REG_NOC_DEBUG_DATA));
	Centurion_Write_Sys_Packet(0, test, 500, 0, 0xAA);
	Centurion_Read_Reg(CENT_REG_NOC_IF_STATUS);
	Centurion_Read_Reg(CENT_REG_NOC_IF_RX_LEN);
	//read(cent_fd, buff, 10);

	Centurion_Write_Sys_Packet(0, test, 50, 0, 0xCA);
	close(cent_fd);
	return 0;
